...
export class RegistrationPage implements OnInit {
  ...
  constructor(public navCtrl: NavController, public alertCtrl: AlertController, public platform: Platform, private auth: AuthenticationService, public loading: LoadingService) { }
  ...
  public register(user): void {
      ...
      this.check(user).then(() => {
        this.auth.register(this.user)
          .then(() => {
            this.navCtrl.navigateRoot('create-company').then(() => {
              this.loading.dismiss();
            })
          }).catch(err => {
            this.presentAlert(err)
          })
      }).catch(err => {console.log("Error: " + err);  this.loading.dismiss();
    })
    })
  }
